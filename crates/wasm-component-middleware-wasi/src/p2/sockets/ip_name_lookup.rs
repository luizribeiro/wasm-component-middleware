...
use wasmtime_wasi::p2::bindings::sync::sockets::ip_name_lookup::{
    ...
    T: WasiView + MiddlewareView + 'static,
{
    ...
    fn drop(&mut self, stream: Resource<ResolveAddressStream>) -> wasmtime::Result<()> {
        gate!(trap self, WASI_VERSION, "wasi:sockets/ip-name-lookup", "[resource-drop]resolve-address-stream", handles = [stream], args = (), delegate = |state: &mut T| HostResolveAddressStream::drop(&mut state.sockets(), stream))
    }
}
