...
use wasmtime_wasi::p2::bindings::sync::sockets::tcp::{
    ...
    T: WasiView + MiddlewareView + 'static,
{
    fn start_bind(
        ...
        gate!(socket self, WASI_VERSION, "wasi:sockets/tcp", "[method]tcp-socket.start-bind", handles = [socket, network], args = [local_address = address(local_address)], delegate = |state: &mut T| HostTcpSocket::start_bind(&mut state.sockets(), socket, network, local_address))
    }
    ...
    fn start_listen(&mut self, socket: Resource<TcpSocket>) -> Result<(), SocketError> {
        gate!(socket self, WASI_VERSION, "wasi:sockets/tcp", "[method]tcp-socket.start-listen", handles = [socket], args = (), delegate = |state: &mut T| HostTcpSocket::start_listen(&mut state.sockets(), socket))
    }
