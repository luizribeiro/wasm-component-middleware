...
use wasmtime_wasi::p2::bindings::sync::sockets::udp::{
...
use wasmtime_wasi::p2::bindings::sync::sockets::udp_create_socket;
...
use super::{address, datagrams, optional_address};
    ...
    T: WasiView + MiddlewareView + 'static,
{
    fn start_bind(
        ...
        gate!(socket self, WASI_VERSION, "wasi:sockets/udp", "[method]udp-socket.start-bind", handles = [socket, network], args = [local_address = address(local_address)], delegate = |state: &mut T| HostUdpSocket::start_bind(&mut state.sockets(), socket, network, local_address))
    }
    ...
    fn stream(
        ...
        SocketError,
    > {
        gate!(socket self, WASI_VERSION, "wasi:sockets/udp", "[method]udp-socket.stream", handles = [socket], args = [remote_address = optional_address(remote_address)], delegate = |state: &mut T| HostUdpSocket::stream(&mut state.sockets(), socket, remote_address), produced = |value: &(Resource<IncomingDatagramStream>, Resource<OutgoingDatagramStream>)| vec![value.0.rep(), value.1.rep()])
    }
    ...
    T: WasiView + MiddlewareView + 'static,
{
    ...
    fn drop(&mut self, stream: Resource<IncomingDatagramStream>) -> wasmtime::Result<()> {
        gate!(trap self, WASI_VERSION, "wasi:sockets/udp", "[resource-drop]incoming-datagram-stream", handles = [stream], args = (), delegate = |state: &mut T| HostIncomingDatagramStream::drop(&mut state.sockets(), stream))
    }
}
    ...
    T: WasiView + MiddlewareView + 'static,
{
    ...
    fn send(
        ...
        gate!(socket self, WASI_VERSION, "wasi:sockets/udp", "[method]outgoing-datagram-stream.send", handles = [stream], args = [datagrams = datagrams(&values)], delegate = |state: &mut T| HostOutgoingDatagramStream::send(&mut state.sockets(), stream, values))
    }
    ...
    fn drop(&mut self, stream: Resource<OutgoingDatagramStream>) -> wasmtime::Result<()> {
        gate!(trap self, WASI_VERSION, "wasi:sockets/udp", "[resource-drop]outgoing-datagram-stream", handles = [stream], args = (), delegate = |state: &mut T| HostOutgoingDatagramStream::drop(&mut state.sockets(), stream))
    }
}
    ...
    T: WasiView + MiddlewareView + 'static,
{
    fn create_udp_socket(
        ...
        gate!(socket self, WASI_VERSION, "wasi:sockets/udp-create-socket", "create-udp-socket", handles = [], args = (address_family), delegate = |state: &mut T| udp_create_socket::Host::create_udp_socket(&mut state.sockets(), address_family), produced = produced_resource)
    }
}
